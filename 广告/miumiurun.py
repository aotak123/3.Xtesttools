...
capabilities['deviceName'] = 'BLA-AL00'
...
def panguan(mobile):
    ...
    try:
        ...
        TouchAction(driver).tap(x=283, y=1174).perform()  # 点击手机号登录
        time.sleep(2)
        ...
        time.sleep(2)
        TouchAction(driver).tap(x=500, y=674).perform()  # 获取验证码
        time.sleep(2)
        ...
        time.sleep(2)
        TouchAction(driver).tap(x=351, y=794).perform()  # 确定按钮登录
    ...
    try:
        ...
        TouchAction(driver).tap(x=634, y=406).perform()  # 关闭签到弹窗
        time.sleep(3)
    # 进入接龙主游戏页面
    TouchAction(driver).tap(x=354, y=1031).perform()  # 进入接龙页面
    time.sleep(3)
    driver.back()
    time.sleep(3)
    # 检查是否弹出签到提示
    try:
        ...
        TouchAction(driver).tap(x=549, y=437).perform()  # 关闭签到提示弹窗
    time.sleep(2)
    # 进入判官进行刷广告
    TouchAction(driver).tap(x=214, y=1183).perform()  # 选择进入判官
    time.sleep(2)
    TouchAction(driver).tap(x=360, y=680).perform()  # 选择开始挑战
    time.sleep(3)
    a = 1
    while a <= 11:
        time.sleep(8)
        TouchAction(driver).tap(x=340, y=871).perform()  # 点击查看广告
        time.sleep(10)
        ...
        try:
            ...
            print(BB)
            time.sleep(40)
            TouchAction(driver).tap(x=660, y=46).perform()  # 关闭toutiao广告
    ...
    time.sleep(2)
    # 退出账号
    TouchAction(driver).tap(x=298, y=74).perform()  # 返回进入个人中心
    time.sleep(2)
    TouchAction(driver).tap(x=654, y=91).perform()  # 点击设置
    time.sleep(2)
    TouchAction(driver).tap(x=360, y=1017).perform()  # 点击退出
    time.sleep(2)
    TouchAction(driver).tap(x=470, y=700).perform()  # 弹窗确定退出
...
if __name__ == '__main__':
    ...
    panguan(18317269850)  # NO.32   小糖糖微信
