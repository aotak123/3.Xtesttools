...
def panguan(mobile):
    ...
    try:
        ...
        TouchAction(driver).tap(x=380, y=1614).perform()  # 点击手机号登录
        time.sleep(2)
        ...
        time.sleep(2)
        TouchAction(driver).tap(x=830, y=595).perform()  # 获取验证码
        time.sleep(2)
        ...
        time.sleep(2)
        TouchAction(driver).tap(x=553, y=846).perform()  # 确定按钮登录
    ...
    try:
        ...
        TouchAction(driver).tap(x=957, y=428).perform()  # 关闭签到弹窗
        time.sleep(3)
    # 进入接龙主游戏页面
    TouchAction(driver).tap(x=536, y=1434).perform()  # 进入接龙页面
    time.sleep(3)
    driver.back()
    time.sleep(3)
    # 检查是否弹出签到提示
    try:
        ...
        TouchAction(driver).tap(x=951, y=500).perform()  # 关闭签到提示弹窗
    time.sleep(2)
    # 进入判官进行刷广告
    TouchAction(driver).tap(x=330, y=1662).perform()  # 选择进入判官
    time.sleep(2)
    TouchAction(driver).tap(x=530, y=953).perform()  # 选择开始挑战
    time.sleep(3)
    a = 1
    while a <= 11:
        time.sleep(8)
        TouchAction(driver).tap(x=542, y=1259).perform()  # 点击查看广告
        time.sleep(10)
        ...
        try:
            ...
            print(BB)
            time.sleep(40)
            TouchAction(driver).tap(x=164, y=109).perform()  # 关闭toutiao广告
    ...
    time.sleep(2)
    # 退出账号
    TouchAction(driver).tap(x=447, y=127).perform()  # 返回进入个人中心
    time.sleep(2)
    TouchAction(driver).tap(x=988, y=180).perform()  # 点击设置
    time.sleep(2)
    TouchAction(driver).tap(x=691, y=1788).perform()  # 点击退出
    time.sleep(2)
    TouchAction(driver).tap(x=735, y=1026).perform()  # 弹窗确定退出
