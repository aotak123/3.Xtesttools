...
# testmobile = '/Users/aotak/Documents/小秀才测试手机号名单.xlsx' #测试记录文件的地址
# realmobile = '/Users/aotak/Documents/小秀才正式手机号名单.xlsx' #正式记录文件的地址

while True:
    type = input("\033[30m \n 请输入验证码类型：\n 1：测试登录\n 2：测试绑定\n 3：正式登录\n 4：正式绑定 \033[0m\n")
    # 3仅支持input输入，不支持raw_input输入
    if type == "1":  # 测试登录
        url = "http://test-u-fkccy.happyxiaoxiucai.com/login/main_login/testtool"
    ...
    if type == "3":
        type = "1"  # 正式登录
        url = "https://u-fkccy.happyxiaoxiucai.com/login/main_login/testtool"
